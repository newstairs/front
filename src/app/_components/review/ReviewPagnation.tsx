...
const ReviewPagination: React.FC<Props> = ({ set_current_page, page_number }) => {
    ...
    const renderPageButtons = () => {
        const startPage = Math.max(1, (currentBlock - 1) * pagesPerBlock + 1);
        const endPage = Math.min(startPage + pagesPerBlock - 1, page_number);
        let buttons = [];
        for (let page = startPage; page <= endPage; page++) {
            buttons.push(
                <button key={page} 
                        className={`p-2 ${page === currentPage ? 'text-red-500' : 'text-black'}`} 
    ...
    // 다음 페이지로 이동
    const handleNext = () => {
        const newPage = Math.min(page_number, currentPage + 1);
        goToPage(newPage);
    };

    return (
        <div className="flex justify-center items-center space-x-2">
            <button disabled={currentPage === 1} onClick={handlePrevious} className="p-2">
                이전
            </button>
            {renderPageButtons()}
            <button disabled={currentPage === page_number} onClick={handleNext} className="p-2">
                다음
            </button>
        </div>
    );
